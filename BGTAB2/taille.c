...
#include <stdlib.h>
#include <SDL2/SDL.h>
#include <SDL2/SDL_image.h> // Include SDL_image header for saving the image
#include <time.h>
...
int main() {
    ...
    unsigned char pixel;
    int nb_oct_par_ligne = 0;  // Nombre d'octets par ligne
    int nb_lines = 0;          // Nombre de lignes

    // Ouvre le fichier en mode binaire
    file = fopen("IMG.BGTAB2.DUN", "rb");
    ...
    // Offset remis au début du fichier
    rewind(file);
    ...
    SDL_Window *window = SDL_CreateWindow("Affichage Image avec SDL",
     ...
     if (renderer == NULL) {
        ...
        // Libérer la surface après l'enregistrement
...
//gcc -o t  taille.c -lSDL2 -lSDL2_image